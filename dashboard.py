...
st.write("Stats vs. League Average")
...
b1.metric("RS/G",r["R/G"],delta = round(float(r["R/G"]) - float(l["R/G"]),2))
b2.metric("Age",r["BatAge"],delta = round(float(r["BatAge"]) - float(l["BatAge"]),2),delta_color = "inverse")
b3.metric("AVG",r["BA"],delta = round(float(r["BA"]) - float(l["BA"]),3))
b4.metric("OBP",r["OBP"],delta = round(float(r["OBP"]) - float(l["OBP"]),3))
b5.metric("SLG",r["SLG"],delta = round(float(r["SLG"]) - float(l["SLG"]),3))
...
r = s[s["Tm"]=="Kansas City Royals"].iloc[0]
l = s[s["Tm"]=="League Average"].iloc[0]
p1.metric("RA/G",r["RA/G"],delta = round(float(r["RA/G"]) - float(l["RA/G"]),2),delta_color="inverse")
p2.metric("Age",r["PAge"],delta = round(float(r["PAge"]) - float(l["PAge"]),2),delta_color="inverse")
p3.metric("K/9",r["SO9"],delta = round(float(r["SO9"]) - float(l["SO9"]),2))
p4.metric("BB/9",r["BB9"],delta = round(float(r["BB9"]) - float(l["BB9"]),2),delta_color="inverse")
p5.metric("HR/9",r["HR9"],delta = round(float(r["HR9"]) - float(l["HR9"]),2),delta_color="inverse")
